...
def create_book(data_ozon, data_wb):
    ...
    page.write('F1', 'Площадка', header)

    row = 1
    column = 0

    for product in data_ozon + data_wb:
        ...
        marketplace = 'Ozon' if row <= len(data_ozon) else 'Wildberries'
        page.write(row, column + 5, marketplace)
        row += 1
